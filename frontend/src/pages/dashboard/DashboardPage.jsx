...
import { getUserBookings, queryDocuments, getUserApplications } from '../../services/firestoreService'
import { formatCurrencyINR } from '../../utils/dummyData'
import {
  CalendarIcon, CurrencyRupeeIcon, BriefcaseIcon, WrenchScrewdriverIcon,
  UserGroupIcon, ClockIcon, CheckCircleIcon, StarIcon, ChartBarIcon,
...
export default function DashboardPage() {
  ...
  const loadDashData = async () => {
    ...
    try {
      ...
      if (isWorker) {
        const svcs = await queryDocuments('services', 'worker_id', '==', user.uid)
        serviceCount = svcs.length
      }
      if (isEmployer) {
        const jobs = await queryDocuments('jobs', 'employer_id', '==', user.uid)
        jobCount = jobs.length
        const gigs = await queryDocuments('gigs', 'employer_id', '==', user.uid)
        gigCount = gigs.length
        // Count total applications received
        let totalApps = 0
        for (const job of jobs) {
          const apps = await queryDocuments('applications', 'jobId', '==', job.id)
  ...
  const getStatCards = () => {
    ...
    // Fallback for roles without specific cards
    if (cards.length === 0) {
      cards.push(
            ...
            {recentBookings.map(b => (
              <Link key={b.id} to={`/bookings/${b.id}`}
                ...
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-primary-50 dark:bg-primary-900/20 rounded-lg flex items-center justify-center text-lg">📋</div>
                  <div>
                ...
                <div className="flex items-center gap-3">
                  <span className={`text-xs px-2 py-0.5 rounded-full font-medium capitalize ${
                    ...
                    b.status === 'in_progress' ? 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-300' :
